...
type AuthHeaders = { userId: string; email: string; displayName: string };

function readAuthHeaders(request: Request): AuthHeaders {
  const userId = request.headers.get("oai-authenticated-user-id");
  const email = request.headers.get("oai-authenticated-user-email");
  if (!userId || !email) throw new AuthenticationError("Faça login para continuar.");
  const encodedName = request.headers.get("oai-authenticated-user-full-name");
  const encoding = request.headers.get("oai-authenticated-user-full-name-encoding");
  let displayName = email.split("@")[0] || "Usuário";
  if (encodedName && encoding === "percent-encoded-utf-8") {
    try { displayName = decodeURIComponent(encodedName); } catch { /* fallback seguro */ }
  }
  return { userId, email: email.toLowerCase(), displayName };
}
...
async function seedSchool(db: D1Database, auth: AuthHeaders): Promise<void> {
  ...
  add("INSERT INTO users (id,school_id,identity_subject,email,display_name,status,mfa_required) VALUES (?,?,?,?,?,'ACTIVE',1)", IDS.actorUser, IDS.school, auth.userId, auth.email, auth.displayName);
